...
class PredizionePartita:
    def __init__(self, squadra_casa, squadra_trasferta, campionatoCorrente='Csv/Campionato.csv', campionatiPrecedenti='Csv/CampionatiPrecedenti.csv', peso_attuale=0.6):
        ...
        # Carica i dati CSV
        print(f"Caricando dati da {campionatoCorrente}...")
        self.dati_corrente = pd.read_csv(campionatoCorrente, sep=';')
        try:
            print(f"Caricando dati storici da {campionatiPrecedenti}...")
            self.dati_storico = pd.read_csv(campionatiPrecedenti, sep=';')
        except:
    ...
    def _calcola_probabilita_avanzata(self):
        ...
        p2 = min(0.85, max(0.15, 0.35 - diff_totale / 200))  # Svantaggio trasferta
        px = max(0.10, 1 - (p1 + p2))
    ...
    def prevedi(self):
        ...
        # print(f"\n--- PREDIZIONE GOAL ---")
        self.predizioneGoals ={'goalPrevistiCasa': goal_casa, 
                               'goalPrevistiTrasferta': goal_trasferta,
                               'esitoPrevisto': esito,'doppiaChance': doppia_chance}
        ...
        # print(f"\n--- PROBABILITA ESITO ---")
        self.probabilitaEsito = {'VittoriaCasa': p1, 'Pareggio': px, 
                            'VittoriaTrasferta': p2}
        
        # print(f"1 (Vittoria {self.squadra_casa}): {p1:.0%}")
        # print(f"X (Pareggio): {px:.0%}")
        # print(f"2 (Vittoria {self.squadra_trasferta}): {p2:.0%}")
        # print(f"\n--- OVER/UNDER ---")
        self.underOver = {'Under1.5': probabilita_under_15, 'Over1.5': probabilita_over_15,
                     'Under2.5': probabilita_under_25, 'Over2.5': probabilita_over_25}
        ...
        self.formaRecente = { 'FormaCasa': forma_casa, 'FormaTrasferta': forma_trasferta}
    ...
    def grafico_confronto(self):
        ...
        plt.tight_layout()
        plt.savefig(myFile.graficoConfrontoSquadre, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def dettaglio_forma(self):
        ...
        print(f"\n=== DETTAGLIO FORMA ===")
        self.analisiFormaCasa = {}
        self.analisiFormaTrasferta = {}
        for squadra in [self.squadra_casa, self.squadra_trasferta]:
            
            if squadra == self.squadra_casa:
                self.analisiFormaCasa['squadra'] = f"{squadra} - Analisi forma:"
            else:
                self.analisiFormaTrasferta['squadra'] = f"{squadra} - Analisi forma:"
            ...
            if len(partite_generali) < 5:
                for _ in range(5 - len(partite_generali)):
                    partiteGenerali.append("- ")
                    print("- ", end="")
            partiteGenerali.append(" ← Usati per calcolo forma")
            partiteGenerali = "".join(partiteGenerali)  # Converti in stringa
            if squadra == self.squadra_casa:
                self.analisiFormaCasa['partiteGenerali'] = partiteGenerali
            else:
                self.analisiFormaTrasferta['partiteGenerali'] = partiteGenerali
            print(" ← Usati per calcolo forma")
            
            forma = self._calcola_forma_squadra(squadra, 'generali')
            if squadra == self.squadra_casa:
                self.analisiFormaCasa['formaGenerale'] = f"  💪 Media punti/partita: {forma:.2f}/3.0"
            else:
                self.analisiFormaTrasferta['formaGenerale'] = f"  💪 Media punti/partita: {forma:.2f}/3.0"
            print(f"  💪 Media punti/partita: {forma:.2f}/3.0")
        # end for
    # fine dettaglio_forma()
    ...
    def _crea_classifica(self, df, where='generali'):
        ...
        for squadra in stats.index:
            ...
            punti = partite = vittorie = pareggi = sconfitte = gf = gs = 0
            
            if where == 'generali':
                punti = int(pd.to_numeric(stats.loc[squadra, 'V']) * 3 + pd.to_numeric(stats.loc[squadra, 'X']))
                partite = int(pd.to_numeric(stats.loc[squadra, 'PG']))
                vittorie = int(pd.to_numeric(stats.loc[squadra, 'V']))
                pareggi = int(pd.to_numeric(stats.loc[squadra, 'X']))
                sconfitte = int(pd.to_numeric(stats.loc[squadra, 'S']))
                gf = int(pd.to_numeric(stats.loc[squadra, 'GF']))
                gs = int(pd.to_numeric(stats.loc[squadra, 'GS']))
            elif where == 'casa':
                punti = int(pd.to_numeric(stats.loc[squadra, 'V_casa']) * 3 + pd.to_numeric(stats.loc[squadra, 'X_casa']))
                partite = int(pd.to_numeric(stats.loc[squadra, 'PG_casa']))
                vittorie = int(pd.to_numeric(stats.loc[squadra, 'V_casa']))
                pareggi = int(pd.to_numeric(stats.loc[squadra, 'X_casa']))
                sconfitte = int(pd.to_numeric(stats.loc[squadra, 'S_casa']))
                gf = int(pd.to_numeric(stats.loc[squadra, 'GF_casa']))
                gs = int(pd.to_numeric(stats.loc[squadra, 'GS_casa']))
            elif where == 'trasferta':
                punti = int(pd.to_numeric(stats.loc[squadra, 'V_trasferta']) * 3 + pd.to_numeric(stats.loc[squadra, 'X_trasferta']))
                partite = int(pd.to_numeric(stats.loc[squadra, 'PG_trasferta']))
                vittorie = int(pd.to_numeric(stats.loc[squadra, 'V_trasferta']))
                pareggi = int(pd.to_numeric(stats.loc[squadra, 'X_trasferta']))
                sconfitte = int(pd.to_numeric(stats.loc[squadra, 'S_trasferta']))
                gf = int(pd.to_numeric(stats.loc[squadra, 'GF_trasferta']))
                gs = int(pd.to_numeric(stats.loc[squadra, 'GS_trasferta']))
    ...
    def _mostra_ranking_campionato_attuale(self):
        ...
        self.casaRankingAttuale = {}
        self.trasfertaRankingAttuale = {}
        ...
        try:
            ...
            self.casaRankingAttuale['generali'] = f"  {self.squadra_casa}: {pos_casa_gen}° posto"
            print(f"  {self.squadra_trasferta}: {pos_trasferta_gen}° posto")
            self.trasfertaRankingAttuale['generali'] = f"  {self.squadra_trasferta}: {pos_trasferta_gen}° posto"
            
            print(f"\n🏠 IN CASA:")
            print(f"  {self.squadra_casa}: {pos_casa_casa}° posto")
            self.casaRankingAttuale['casa'] = f"  {self.squadra_casa}: {pos_casa_casa}° posto"
            print(f"  {self.squadra_trasferta}: {pos_trasferta_casa}° posto")
            self.trasfertaRankingAttuale['casa'] = f"  {self.squadra_trasferta}: {pos_trasferta_casa}° posto"
            
            print(f"\n✈️  IN TRASFERTA:")
            print(f"  {self.squadra_casa}: {pos_casa_trasf}° posto")
            self.casaRankingAttuale['trasferta'] = f"  {self.squadra_casa}: {pos_casa_trasf}° posto"
            print(f"  {self.squadra_trasferta}: {pos_trasferta_trasf}° posto")
            self.trasfertaRankingAttuale['trasferta'] = f"  {self.squadra_trasferta}: {pos_trasferta_trasf}° posto"
    ...
    def _mostra_ranking_ultime_5_partite(self):
        ...
        self.casaRankingUltime5 = {}
        self.trasfertaRankingUltime5 = {}
        ...
        try:
            ...
            self.casaRankingUltime5['generali'] = f"  {self.squadra_casa}: {casa_pos_gen}° posto"
            print(f"  {self.squadra_trasferta}: {trasf_pos_gen}° posto")
            self.trasfertaRankingUltime5['generali'] = f"  {self.squadra_trasferta}: {trasf_pos_gen}° posto"
            ...
            self.casaRankingUltime5['casa'] = f"  {self.squadra_casa}: {casa_pos_casa}° posto"
            print(f"  {self.squadra_trasferta}: {trasf_pos_casa}° posto")
            self.trasfertaRankingUltime5['casa'] = f"  {self.squadra_trasferta}: {trasf_pos_casa}° posto"
            ...
            self.casaRankingUltime5['trasferta'] = f"  {self.squadra_casa}: {casa_pos_trasf}° posto"
            print(f"  {self.squadra_trasferta}: {trasf_pos_trasf}° posto")
            self.trasfertaRankingUltime5['trasferta'] = f"  {self.squadra_trasferta}: {trasf_pos_trasf}° posto"
    ...
    def _mostra_ranking_storico(self):
        ...
        self.casaRankingStorico = {}
        self.trasfertaRankingStorico = {}
        
        messageSquadraDiCasaNonTrovata = f"⚠️  {self.squadra_casa} non presente nei campionati precedenti"
        messageSquadraInTrasfertaNonTrovata = f"⚠️  {self.squadra_trasferta} non presente nei campionati precedenti"
        ...
        try:
            ...
            if casa_disponibile:
                ...
                self.casaRankingStorico['generale'] = f"  {self.squadra_casa}: {pos_casa_gen}° posto"
                print(f"  {self.squadra_casa}: {pos_casa_gen}° posto")
            if trasferta_disponibile:
                ...
                self.trasfertaRankingStorico['generale'] = f"  {self.squadra_trasferta}: {pos_trasferta_gen}° posto"
                print(f"  {self.squadra_trasferta}: {pos_trasferta_gen}° posto")
            ...
            if casa_disponibile:
                ...
                self.casaRankingStorico['casa'] = f"  {self.squadra_casa}: {pos_casa_casa}° posto"
                print(f"  {self.squadra_casa}: {pos_casa_casa}° posto")
            if trasferta_disponibile:
                pos_trasferta_casa = classifica_storica_casa[classifica_storica_casa['Squadra'] == self.squadra_trasferta]['Posizione'].iloc[0]
                self.trasfertaRankingStorico['casa'] = f"  {self.squadra_trasferta}: {pos_trasferta_casa}° posto"
                print(f"  {self.squadra_trasferta}: {pos_trasferta_casa}° posto")
            ...
            if casa_disponibile:
                ...
                self.casaRankingStorico['trasferta'] = f"  {self.squadra_casa}: {pos_casa_trasf}° posto"
                print(f"  {self.squadra_casa}: {pos_casa_trasf}° posto")
            if trasferta_disponibile:
                pos_trasferta_trasf = classifica_storica_trasf[classifica_storica_trasf['Squadra'] == self.squadra_trasferta]['Posizione'].iloc[0]
                self.trasfertaRankingStorico['trasferta'] = f"  {self.squadra_trasferta}: {pos_trasferta_trasf}° posto"
                print(f"  {self.squadra_trasferta}: {pos_trasferta_trasf}° posto")
            ...
            if casa_disponibile or trasferta_disponibile:
                ...
                if casa_disponibile:
                    ...
                    self.casaRankingStorico['statisticheAggregate'] = f"  {self.squadra_casa}: {int(casa_stats['PG'])} partite, {int(casa_stats['V'])}-{int(casa_stats['X'])}-{int(casa_stats['S'])}, {int(casa_stats['GF'])}-{int(casa_stats['GS'])}"
                ...
                if trasferta_disponibile:
                    ...
                    self.trasfertaRankingStorico['statisticheAggregate'] = f"  {self.squadra_trasferta}: {int(trasf_stats['PG'])} partite, {int(trasf_stats['V'])}-{int(trasf_stats['X'])}-{int(trasf_stats['S'])}, {int(trasf_stats['GF'])}-{int(trasf_stats['GS'])}"
    ...
    def _mostra_scontri_diretti_storici(self):
        ...
        class ScontriDiretti:
            numeroScontri = ''
            bilancioScontri = {}
            ultime5Partite = {}
        
        self.scontri = ScontriDiretti()
        ...
        try:
            ...
            self.scontri.numeroScontri = f"Trovati {len(scontri_diretti)} scontri diretti storici"
            ...
            self.scontri.bilancioScontri['casa'] = f"  {self.squadra_casa}: {vittorie_casa} vittorie"
            self.scontri.bilancioScontri['trasferta'] = f"  {self.squadra_trasferta}: {vittorie_trasferta} vittorie"
            self.scontri.bilancioScontri['pareggi'] = f"  Pareggi: {pareggi}"
            ...
            for _, partita in ultime_5_scontri.iterrows():
                ...
                self.scontri.ultime5Partite[index] = f"  {data}: {risultato}"
                index += 1
    ...
    def _analisi_ai_e_consigli(self, p1, px, p2, forma_casa, forma_trasferta, goal_totali, prob_under_25, prob_over_25):
        ...
        try:
            ...
            self.scommessaConsigliata = {}
            
            self.scommessaConsigliata['doppiaChance'] = scommessa_principale['nome']
            self.scommessaConsigliata['probabilita'] = f"{scommessa_principale['probabilita']:.0%}"
            self.scommessaConsigliata['livelloRischio'] = livello_rischio
            ...
            self.motivazioneTecnica['predizioneEsito'] = f"{favorito} favorito con {prob_favorito:.0%} di probabilità"
            
            print(f"📊 Predizione esito: {favorito} favorito con {prob_favorito:.0%} di probabilità")
            self.motivazioneTecnica['goalsPrevisti'] = f"{self.squadra_casa} vs {self.squadra_trasferta} → {goal_totali:.1f} goal totali"
            ...
            self.motivazioneTecnica['formaRecente'] = forma_analisi
            ...
            self.motivazioneTecnica['rankingAttuale'] = ranking_analisi
            print(f"🏆 Ranking attuale: {ranking_analisi}")
            
            # Scontri diretti
            if scontri_info:
                self.motivazioneTecnica['scontriDiretti'] = scontri_info
    ...
    def _raccomandazioni_aggiuntive(self, prob_under_25, prob_over_25, p1, px, p2, forma_casa, forma_trasferta):
        ...
        if max_prob >= 0.60 and prob_under_25 >= 0.55:
            if p1 == max_prob:
                self.comboSuggerita = f"💡 Combo suggerita: 1 + Under 2.5"
                print(f"💡 Combo suggerita: 1 + Under 2.5")
            elif p2 == max_prob:
                self.comboSuggerita = f"💡 Combo suggerita: 2 + Under 2.5"
                print(f"💡 Combo suggerita: 2 + Under 2.5")
            else:
                self.comboSuggerita = f"💡 Combo suggerita: X + Under 2.5"
    ...
    def _commento_finale_ai(self, scommessa_principale, p1, px, p2, forma_casa, forma_trasferta, pos_casa, pos_trasferta):
        ...
        self.commentoFinale['analisiContesto'] = commento
        print(commento)
        
        # Consiglio finale basato sulla probabilità
        prob_successo = scommessa_principale['probabilita']
        if prob_successo >= 0.75:
            self.commentoFinale['confidenza'] = "🟢 **Confidenza ALTA** - Scommessa consigliata con ottime probabilità di successo."
            print(f"🟢 **Confidenza ALTA** - Scommessa consigliata con ottime probabilità di successo.")
        elif prob_successo >= 0.65:
            self.commentoFinale['confidenza'] = "🟡 **Confidenza MEDIA** - Scommessa equilibrata, buon rapporto rischio/rendimento."
            print(f"🟡 **Confidenza MEDIA** - Scommessa equilibrata, buon rapporto rischio/rendimento.")
        else:
            self.commentoFinale['confidenza'] = "🔴 **Confidenza BASSA** - Partita incerta, considera scommesse più conservative."
...
if __name__ == "__main__":
    ...
    predizione = PredizionePartita('Inter', 'Milan', peso_attuale=0.7)
    predizione.prevedi()
